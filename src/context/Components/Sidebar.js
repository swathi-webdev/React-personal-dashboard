  ...
  return (
    <>
      ...
      <button
        ...
        style={{ top: 12, left: 12, zIndex: 1050, background: '#1f2d3d', border: '1px solid #253040', color: '#d4dce8' }}
      >
        {open ? <X size={20} /> : <Menu size={20} />}
      </button>

      <aside className={`sidebar d-flex flex-column ${open ? 'open' : ''}`}>
        <div className="p-3 d-flex align-items-center gap-2 border-bottom" style={{ borderColor: '#253040' }}>
          <div className="bg-primary-custom rounded d-flex align-items-center justify-content-center" style={{ width: 32, height: 32 }}>
            <LayoutDashboard size={16} color="#1a2332" />
          </div>
